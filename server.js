...
const JWT_SECRET = process.env.JWT_SECRET || 'YOUR_SECURE_KEY';
...
let systemSock = null; // Single System Bot
...
db.serialize(() => {
    ...
    const addColumn = (t, c, type) => { 
        db.run(`ALTER TABLE ${t} ADD COLUMN ${c} ${type}`, () => {}); 
    };
...
function processSpintax(text) { if (!text) return ""; return text.replace(/\{([^{}]+)\}/g, (m, o) => { const c = o.split('|'); return c[Math.floor(Math.random() * c.length)]; }); }
...
io.on('connection', (socket) => {
    ...
    socket.on('init-whatsapp', async (token) => {
        try {
            ...
            // Check if actual session file exists
            ...
            if (existing && existing.user && hasSession) {
    ...
    // =======================================================
    // === SMART FILTER (PROFILE PIC + STOP + FIXED) =========
    // =======================================================
    socket.on('check-numbers', async ({ numbers }) => {
        if (!systemSock || !systemSock.user) {
            return socket.emit('filter-error', 'System Bot غير متصل! المرجو مسح QR في السيرفر.');
        }
    ...
    socket.on('send-promo', async (data) => {
        ...
        if(!sock) return socket.emit('send-promo-status', {success:false, phone, error:'Not Connected'});
...
app.get('/api/auth/verify-email', (req, res) => {
    try {
        ...
        db.run("INSERT INTO users (id,email,name,password,trialEndsAt) VALUES (?,?,?,?,?)", [id,p.email,p.name,p.password,p.trialEndsAt], (err) => {
            if(err) return res.status(500).send('Error'); delete pendingRegistrations[email]; res.send('Active! <a href="/login.html">Login</a>');
        });
...
app.post("/api/auth/login", async (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE email=?", [email], async (err, user) => {
        if (!user || !user.password || !(await bcrypt.compare(password, user.password))) return res.status(401).json({ message: 'خطأ' });
    ...
    fs.createReadStream(req.file.path)
        ...
        .on('end', () => {
            ...
            db.serialize(() => {
                ...
                stmt.finalize();
                db.run("COMMIT", () => res.json({ message: "Imported", imported: importedCount }));
            });
        });
});

app.get("/contacts", authMiddleware, checkSubscription, (req, res) => db.all(`SELECT * FROM clients WHERE ownerId=?`, [req.userData.userId], (e,r)=>res.json(r||[])));
app.get("/imported-contacts", authMiddleware, checkSubscription, (req, res) => db.all(`SELECT * FROM imported_clients WHERE ownerId=?`, [req.userData.userId], (e,r)=>res.json(r||[])));
...
app.post("/addPromo", authMiddleware, checkSubscription, uploadPromoImage.single("image"), (req, res) => {
    try {
        ...
        promos.push({ id: Date.now(), text: req.body.text, image: req.file ? req.file.filename : null });
        writePromos(req.userData.userId, promos);
...
app.delete("/api/delete-all-imported", authMiddleware, checkSubscription, (req, res) => { db.run(`DELETE FROM imported_clients WHERE ownerId=?`, [req.userData.userId], ()=>res.json({ success: true })); });

// Blog (Admin Only)
app.get('/api/blog-posts', (req, res) => { try{ res.json(JSON.parse(fs.readFileSync(blogFile))); } catch(e){res.json([]);} });
app.post('/api/blog-post', authMiddleware, (req, res) => { checkAdmin(req.userData.userId, (isAdmin) => { if(!isAdmin) return res.status(403).json({message:"Forbidden"}); const p=fs.existsSync(blogFile)?JSON.parse(fs.readFileSync(blogFile)):[]; p.unshift({...req.body, id:Date.now(), date:new Date().toISOString().split('T')[0]}); fs.writeFileSync(blogFile, JSON.stringify(p)); res.json({success:true}); }); });
app.delete('/api/blog-post/:id', authMiddleware, (req, res) => { checkAdmin(req.userData.userId, (isAdmin) => { if(!isAdmin) return res.status(403).json({message:"Forbidden"}); let p=JSON.parse(fs.readFileSync(blogFile)); p=p.filter(x=>x.id!==parseInt(req.params.id)); fs.writeFileSync(blogFile, JSON.stringify(p)); res.json({success:true}); }); });
